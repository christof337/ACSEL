...
START_TEST(test_stochastic_rounding) {
	...
	// for ( long int initialPrecision = MPFR_PREC_MIN ; initialPrecision <= MPFR_PREC_MAX ; ++initialPrecision ) {
		...
		for ( long int precisionToRoundTo = MPFR_PREC_MIN ; precisionToRoundTo <= MPFR_PREC_MAX ; ++precisionToRoundTo ) {
			sprintf(logBuffer,"[%ld]\t[%ld] - Target precision is now `%ld`\n",initialPrecision,precisionToRoundTo,precisionToRoundTo);
			m_log(logFile,logBuffer);
			int flagSign = 0;
			m_init2(valToRound, initialPrecision);

			sprintf(logBuffer,"[%ld]\t[%ld] - Begin of changing the values...\n",initialPrecision,precisionToRoundTo);
			m_log(logFile,logBuffer);
			...
			for ( int j = 0 ; j < NB_VALUES; ++j ) {
				...
				if ( initialPrecision < precisionToRoundTo ) {
					sprintf(logBuffer,"-----[%ld]\t[%ld]\t{%s} - Target precision (%ld) is greater than initialPrecision (%ld). Simple rounding.\n",initialPrecision,precisionToRoundTo,logCurrentVal,precisionToRoundTo,initialPrecision);
					m_log(logFile,logBuffer);
					...
					sprintf(logBuffer,"[%ld]\t[%ld]\t{%s} - Begin of loop over multiple stochastic roundings...\n",initialPrecision,precisionToRoundTo,logCurrentVal);
					m_log(logFile,logBuffer);
					...
					mpfr_t expectedPctRoundUp; m_init2(expectedPctRoundUp,mpfr_get_default_prec());
					m_div(expectedPctRoundUp,lastDigits,maxValue,MPFR_RNDN);

					mpfr_t lowerBound; mpfr_init2(lowerBound,mpfr_get_default_prec());
					mpfr_sub_d(lowerBound,expectedPctRoundUp,ERROR_MARGIN,MPFR_RNDN);
					mpfr_t upperBound; mpfr_init2(upperBound,mpfr_get_default_prec());
					mpfr_add_d(upperBound,expectedPctRoundUp,ERROR_MARGIN,MPFR_RNDN);

					mpfr_t actualPctRoundUp; mpfr_init2(actualPctRoundUp,mpfr_get_default_prec());
					mpfr_set_d(actualPctRoundUp,(nbRoundUp/NB_ITER),MPFR_RNDN);

					sprintf(logBuffer,"---------[%ld]\t[%ld]\t{%s} - `%d` rounding upwards, upon `%d` roundings (%4f%s)\nShould be %4f%s\n",initialPrecision,precisionToRoundTo,logCurrentVal,nbRoundUp,NB_ITER,mpfr_get_d(actualPctRoundUp,MPFR_RNDN)*100,"%",mpfr_get_d(expectedPctRoundUp,MPFR_RNDN)*100,"%");
					m_log(logFile,logBuffer);
					ck_assert ( mpfr_greaterequal_p(actualPctRoundUp,lowerBound) || mpfr_lessequal_p (actualPctRoundUp,upperBound) );
