...
void testExtendedRounding(mpfr_t * valToRound, mpfr_t* actualResult,
		long int initialPrecision, long int precisionToRoundTo, int j,
		FILE* logBuffer) {
	...
	fprintf(logBuffer,
			...
			initialPrecision, precisionToRoundTo, j,
			mpfr_get_d(*valToRound, MPFR_RNDN), precisionToRoundTo,
			initialPrecision);
	...
	if (mpfr_equal_p(*actualResult, expectedResult)) {
		...
		fprintf(stderr,
				...
				mpfr_get_d(*valToRound, MPFR_RNDN),
				mpfr_get_d(*actualResult, MPFR_RNDN),
				mpfr_get_d(*valToRound, MPFR_RNDN));
	}
}
...
int test_stochastic_rounding() {
	...
	for (long int initialPrecision = MPFR_PREC_MIN + 10;	initialPrecision < MAX_TESTED_PRECISION /*100*/; ++initialPrecision) {
		...
		for (long int precisionToRoundTo = MPFR_PREC_MIN+7; precisionToRoundTo < MAX_TESTED_PRECISION /*100*/; ++precisionToRoundTo) {
			fprintf(logBuffer, "[%ld]\t[%ld] - Target precision is now `%ld`. ",
			...
			for (int j = 0; j < NB_VALUES; ++j) {
				...
				generateRandomValueInPrecisionRange(&valToRound,
						initialPrecision, maxValue,	flagSign);
				...
				fprintf(logBuffer,
						"[%ld]\t[%ld]\t(%d) - Starting to test the value `%f`. ",
						initialPrecision, precisionToRoundTo, j,
						mpfr_get_d(valToRound, MPFR_RNDN));
				...
				if (initialPrecision <= precisionToRoundTo) {
					// rounding to a greater precision ==> no need for actual rounding
					testExtendedRounding(&valToRound, &actualResult,
							initialPrecision, precisionToRoundTo, j, logBuffer);
					...
					fprintf(logBuffer,
							"Begin of loop over multiple stochastic roundings...\n");
					for (long int k = 0; k < NB_ITER; ++k) {
						...
						roundingDirection = stochasticRounding(&actualResult,
								precisionToRoundTo);
					...
					mpfr_fprintf(logBuffer, "\tShould be %RF/%RF=%4.4Rf%1s\n",
							lastDigits, maxLastDigitsValue, expectedPctRoundUp,
							"%");
					++nbRnd;
